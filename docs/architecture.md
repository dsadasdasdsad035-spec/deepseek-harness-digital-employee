...
Submission calls the Host `digitalEmployees.startChat` operation with a caller-generated Session ID, one submission identity, and the first text or image content. The Host validates current employee and template availability, resolves the existing employee composition, creates the root Agent, admits the standard first user message, and returns the Session only after acceptance. Failed startup disposes unpublished work; the client retains the mention, draft, and attachments and navigates only after success.
