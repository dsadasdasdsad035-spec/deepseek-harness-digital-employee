...
| [`core/digital-employee`](../packages/core/digital-employee/README.zh.md) | 带版本的员工模板与持久化实例操作 | `ctx.digitalEmployees` |
...
数字员工客户端向通用新任务编辑器贡献一个路由引用。该引用携带稳定的员工实例 ID，仅在首个语义位置有效，并由所属插件消费，而不会序列化进模型可见文本。管理工作区的**开始聊天**操作会打开同一条编辑器路径并预选员工，不会创建 Session。

提交时会调用 Host 的 `digitalEmployees.startChat` 操作，并携带调用方生成的 Session ID、一个提交身份，以及首条文本或图像内容。Host 验证员工与模板当前可用性，解析既有员工组合，创建根 Agent，接纳标准的首条用户消息，并且只在接纳后返回 Session。启动失败会 dispose 未发布的工作；客户端保留员工提及、草稿与附件，并且只在成功后导航。

必读的 `digital-employee/identity` Session 事件会在发布前记录创建时的员工、模板、版本和解析后组合身份。首项任务内容使用普通 `user/message` 事件，因此实例元数据变化后，回放仍会保留员工归属与标准会话历史。
...
| 添加持久化数字员工组合 | 在 `ctx.digitalEmployees` 上注册带版本的模板；挂载一个 Provider 与 Agent Consumer |
| 添加聊天原生的数字员工入口 | 贡献一个前置路由引用，并调用原子化的 `digitalEmployees.startChat` 操作 |
