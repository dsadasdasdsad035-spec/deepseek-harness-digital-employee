...
| [notification.md](notification.zh.md) | 通道注册表式通知 seam：`NotificationMessage`、`NotificationChannel`、`NotificationDelivery`、三个 webhook 通道 provider 与失败可见的投递结果 |
...
| [email.md](email.zh.md) | 邮件 seam：`EmailMessage`、`EmailTransport`、`EmailDelivery` 与凭证引用解析的 QQ SMTP transport |
| [user-accounts.md](user-accounts.zh.md) | 账户系统：SQLite 用户存储、argon2id 哈希、验证码、重置令牌与签名会话 |
