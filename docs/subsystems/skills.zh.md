...
<a id="ctxskillmarket--skillmarketgateway"></a>

### `ctx.skillMarket` — `SkillMarketGateway`
 ...
 */
@Remote('banner') async banner(request: SkillMarketBannerRequest): Promise<SkillMarketBannerResult>
 ...
 */
@Remote('install') async install(request: SkillMarketInstallRequest): Promise<SkillMarketInstallResult>

/**
 * List managed marketplace installations.
 * @returns deterministic managed inventory, or a declared domain failure.
 */
@Remote('list') async list(): Promise<SkillMarketListResult>
 ...
 */
@Remote('uninstall') async uninstall(request: SkillMarketUninstallRequest): Promise<SkillMarketUninstallResult>
```
...
/**
 * Restrict the complete skill catalog for the calling scope.
 * Restrictions intersect and apply after global and scoped providers merge.
 * @param filter - exact skill allowlist; an empty list denies every skill.
...
<a id="skill-filesystemhost-mutation--emit"></a>

#### `skill-filesystem/host-mutation` — emit
