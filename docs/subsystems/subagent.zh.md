...
interface SubagentStartRequest {
  ...
  /**
   * Non-secret, losslessly JSON-serializable composition data consumed by
   * `subagent/compose` listeners in the unpublished child Agent context.
   * Continuable children persist this record and replay it on cold resume.
   */
...
<a id="subagentcompose--serial"></a>

#### `subagent/compose` — serial

Compose plugin-owned, non-secret data into an unpublished child Agent.

```ts cordis-catalog
/**
 * Compose plugin-owned, non-secret data into an unpublished child Agent.
 * @param childCtx - unpublished child Agent context.
 * @param data - detached JSON composition record.
 * @mode serial
 */
'subagent/compose'(childCtx: Context, data: Readonly<Record<string, JsonValue>>): Promise<void> | void
```
