...
| [notification.md](notification.md) | the channel-registry notification seam: `NotificationMessage`, `NotificationChannel`, `NotificationDelivery`, three webhook channel providers, and fail-visible delivery outcomes |
...
| [email.md](email.md) | the email seam: `EmailMessage`, `EmailTransport`, `EmailDelivery`, and the QQ SMTP transport with credential-reference resolution |
| [user-accounts.md](user-accounts.md) | the account system: SQLite users, argon2id hashing, verification codes, reset tokens, and signed sessions |
