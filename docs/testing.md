...
Digital employee package tests cover registration, persistence, lifecycle, upgrade, and authority; the keyless snapshot covers model-visible behavior; Web coverage owns management journeys. New Session events update both SDK projections. The marketplace reference adds focused archive and trust tests, a capability snapshot, and Web installation-through-`@` coverage.
