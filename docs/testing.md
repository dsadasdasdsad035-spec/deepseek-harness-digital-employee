...
Every non-trivial model-, protocol-, or human-visible change adds or updates a keyless scenario in the same PR through a runnable example's owning snapshot suite. Package tests, e2e assertions, mocks, and test-only compositions do not replace the assembled transcript; extend the harness when needed. ACP scenarios use `examples/<name>/tests/snapshots/` over the [`dsh-acp-snapshot`](../packages/test-support/acp-snapshot/README.md) factory (`examples/acp-agent` is primary); `examples/headless-agent` owns canonical-event JSONL snapshots and replay fixtures. The `pwsh-tool-turn` scenario boots real `pwsh` and skips where absent. Completed terminal journeys use `apps/cli/tests/snapshots/`; transient presentation uses package semantic tests, plus PTY coverage when input, Loader selection, or teardown changes. Browser journeys use `apps/web/tests/snapshots/`. The two SDKs project the agent loop, session lifecycle, and `SessionEventMap` independently, so changing any updates both: `examples/jsonrpc-agent/tests/snapshots/` owns TypeScript; `scripts/snapshots/python-sdk-single-exe/` owns Python and runs in required `python-runtime` CI.

Digital employee changes cover template registration, provider persistence, lifecycle, upgrade, and authority intersections in package tests; model-visible identity, memory, expert delegation or denial, and memory decisions in a keyless assembled snapshot; and management journeys in Web browser coverage. New digital employee Session events update both SDK projections.
