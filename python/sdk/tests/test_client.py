...
def test_high_level_sdk_preserves_digital_employee_identity_from_runtime(
    tmp_path: Path,
) -> None:
    script = tmp_path / "digital_employee_runtime.py"
    script.write_text(
        """
import json
import sys
...
for line in sys.stdin:
    ...
    if method == "initialize":
        ...
        session_id = (msg.get("params") or {})["sessionId"]
        print(json.dumps({"jsonrpc": "2.0", "method": "session.event", "params": {"sessionId": session_id, "event": {"type": "agent/inbox/spliced", "seq": 0, "time": 0, "data": {"target": "next-turn", "start": 0, "inserted": [{"id": "message-1"}]}}}}), flush=True)
        print(json.dumps({"jsonrpc": "2.0", "id": msg["id"], "result": {"messageId": "message-1"}}), flush=True)
        print(json.dumps({"jsonrpc": "2.0", "method": "session.event", "params": {"sessionId": session_id, "event": {"type": "digital-employee/identity", "seq": 1, "time": 0, "data": {"employeeId": "employee-1", "templateId": "template-1", "templateVersion": "1.0.0", "compositionId": "sha256:8be41a8f3ec34d2b8db322d4f36c77704f560391c265e51f424bed7a427a33f8", "displayName": "Release Engineer", "personality": "Methodical"}}}}), flush=True)
        print(json.dumps({"jsonrpc": "2.0", "method": "session.status", "params": {"sessionId": session_id, "status": "idle"}}), flush=True)
    elif method == "shutdown":
        print(json.dumps({"jsonrpc": "2.0", "id": msg["id"], "result": {}}), flush=True)
        break
""".strip()
    ...
    with DeepSeekHarness(
        ...
        result = harness.run("operate as the employee", session_id="main")
    ...
    assert result.events[1] == {
        ...
        "data": {
            ...
            "compositionId": "sha256:8be41a8f3ec34d2b8db322d4f36c77704f560391c265e51f424bed7a427a33f8",
