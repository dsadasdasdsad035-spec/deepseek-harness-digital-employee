...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-skill-market': { kind: 'none', reason: 'Browser-side marketplace management registers no prompt, tool, session event, or model request input.' },
  ...
  'packages/skill/skill-market': { kind: 'none', reason: 'Host-side marketplace validation and filesystem mutations register no prompt, tool, session event, or model request input.' },
  'packages/mcp/mcp-market': { kind: 'none', reason: 'Host-side package lifecycle and MCP credential-reference configuration register no model request input by themselves.' },
  'packages/tool/tool-market': { kind: 'none', reason: 'Host-side package lifecycle registers no model request input until an installed plugin activates in a later composition.' },
  'packages/util/marketplace-core': { kind: 'none', reason: 'Archive validation and managed filesystem primitives run outside model request assembly.' },
