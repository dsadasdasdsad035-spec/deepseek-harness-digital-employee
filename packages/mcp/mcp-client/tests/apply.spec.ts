...
describe('mcp-client plugin module exports', () => {
  ...
  it('Config schema accepts a valid serverName', () => {
    ...
    if ('mode' in resolved) throw new Error('expected static MCP server configuration')
    expect(resolved.serverName).toBe('github-prod_1')
  })

  it('Config schema accepts manager mode without a server connection', () => {
    expect(ConfigSchema({ mode: 'manager' } as never)).toEqual({ mode: 'manager' })
  })
  ...
  it('Config schema materializes reconnect defaults and merges partial overrides', () => {
    ...
    if ('mode' in omitted) throw new Error('expected static MCP server configuration')
    ...
    if ('mode' in partial) throw new Error('expected static MCP server configuration')
...
describe('apply (plugin lifecycle)', () => {
  ...
  it('mounts context-owned MCP connections through manager mode', async () => {
    await apply(ctx, { mode: 'manager' })
    expect(mockConnect).not.toHaveBeenCalled()

    const dispose = await ctx.mcpClients.mount(ctx, stdioConfig)
