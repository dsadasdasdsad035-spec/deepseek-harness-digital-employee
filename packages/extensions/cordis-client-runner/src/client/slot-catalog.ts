...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    keyDomain: 'fixed by the owner\'s key table { [Kind in ChatNodeKind]: { node: ChatNode<Kind> } }, already taken: assistant-step, command, command-input, compaction, company-group-message, company-group-turn, context, manual-compaction, model-retry, steering, tool-call, turn-error, turn-max-tokens, turn-tail, unknown, user, workflow-run',
    hookContext: 'string',
    ...
    occupants: [
      'client-ui-companies GroupMessageNodeView key \'company-group-message\'',
      'client-ui-companies GroupTurnNodeView key \'company-group-turn\'',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-digital-employees NotificationSettingsRow id \'notification-channels\'',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-skill-market SkillMarketSection id \'skill-market\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    kind: 'single',
    scope: 'root',
    summary: 'Root-scoped operational workspace shown in the center column when `ctx.layout.openApplication()` is active.',
    doc: 'Root-scoped operational workspace shown in the center column when\n`ctx.layout.openApplication()` is active.',
    registerOptions: [],
    ownerProps: [
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      'client-ui-digital-employees DigitalEmployeeWorkspace',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      'client-ui-companies CompanyWorkspace id \'company-console\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'sidebar\' (client-ui-sidebar), so it exists while that entry is mounted',
    occupants: [
      'client-ui-companies OrganizationNav id \'organization\'',
