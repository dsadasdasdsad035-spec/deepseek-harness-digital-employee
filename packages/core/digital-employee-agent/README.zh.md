...
当 `createTask()` 收到记忆查询时，它会在创建 Session 前解析一份有界、归员工所有的投影。设置过程记录 `digital-employee/memory-projection`，包含每条可见记忆的 ID、作用域、渲染内容与来源，再从同一个事件 payload 渲染提示词片段。`resumeTask()` 接受同样的可选查询，并在恢复 Session 的下一轮之前追加一份新投影，因此冷恢复的会话能看到当前记忆。

每个员工 Agent 作用域都会挂载 `employee_memory` 工具：`save` 通过受控提升提交一条长期候选（带会话来源），`search` 返回该员工自有记录的有界结果。策略拒绝（内容重复、敏感被禁用、保留期超限）会作为工具结果返回给模型，且每个决定都会以 `digital-employee/memory-decision` 记录在会话日志上。该工具是作用域本地注册，按工具注册表的契约在业务工具限制之后仍保持模型可见。
...
已授权的技能必须到达模型，而不只是进入注册表。员工组装会把工具收窄到声明的业务工具，但始终把技能能力缝的加载器工具（`skill`）保留在该限制之外——因为该缝把加载器与模型可见的会话技能目录一起发布，加载器一旦被遮蔽，两者同时消失。声明了技能却运行在未挂加载器的 preset 上的员工会在组合期失败，诊断会指出缺失的插件，而不是组装出一个模型看不到其技能的员工。
...
每次员工组合还会向该员工的持久审计日志写入能力归属记录。当前 Agent 从组合作用域以 agent-loop 暴露的属性（`extend({ agent })`）读取，绝不作为已注册服务；作用域未暴露 Agent 时组合失败，而不是静默跳过审计。正是这份归属让控制台能把员工的运行中会话——包括其公司群成员会话——映射回该员工，从而在员工于会话中干活时于其工位呈现为在忙。
