...
import { errorChain, type UserMessage } from '@deepseek-ai/dsh-llm'
...
export class AgentLoop extends Service implements AgentFactory {
  ...
  private async setupAndPublish(
    ...
    initialMessages: readonly UserMessage[] | undefined,
    ...
    try {
      ...
      for (const message of initialMessages ?? []) {
        prepared.agent.inbox.append('next-turn', message)
      }
      setupCommit?.commit()
      const handle = prepared.publish(source)
      if ((initialMessages?.length ?? 0) > 0) prepared.agent.wakePending()
      return handle
    } catch (error: unknown) {
