...
const USAGE = [
  'Usage: dsh-market-package <source-directory> --kind tool|mcp|hook|workflow|subagent|employee --publisher-id <id>',
...
function parseArguments(args: readonly string[]): ParsedArguments {
  ...
  if (values.kind !== 'tool' && values.kind !== 'mcp' && values.kind !== 'hook' && values.kind !== 'workflow' && values.kind !== 'subagent' && values.kind !== 'employee') throw new Error('--kind must be tool, mcp, hook, workflow, or subagent')
