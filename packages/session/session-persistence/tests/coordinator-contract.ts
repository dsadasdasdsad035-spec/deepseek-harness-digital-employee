...
export function runCoordinatorContract(name: string, makeFixture: () => Promise<CoordinatorFixture>): void {
  describe(`PersistenceCoordinator orchestration: ${name}`, () => {
    ...
    it('restores required digital employee identity events as known durable input', async () => {
      const fix = await makeFixture()
      const { ctx, fiber } = await freshCtx(fix)
      try {
        const m = meta('digital-employee-identity', WORK)
        const identity = {
          ...
          data: {
            ...
            templateId: createDigitalEmployeeTemplateId('release-engineer'),
            templateVersion: '1.0.0',
            compositionId: createDigitalEmployeeCompositionId(
              'sha256:8be41a8f3ec34d2b8db322d4f36c77704f560391c265e51f424bed7a427a33f8',
            ),
            displayName: 'Release Engineer',
            personality: 'Methodical',
          },
        } satisfies SessionEvent<'digital-employee/identity'>
        await ctx.sessionPersistence.create(m)
        await ctx.sessionPersistence.append(m.id, [...oneTurnLog(), identity])

        const loaded = await ctx.sessionPersistence.load(m.id)

        expect(loaded.events.at(-1)).toEqual(identity)
      } finally {
        await fiber.dispose()
        await fix.cleanup()
      }
