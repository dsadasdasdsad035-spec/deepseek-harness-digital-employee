...
export interface ReferenceInsert {
  ...
  readonly ref: string
  /**
   * Routing references claim composer submission for their owning source and
   * are omitted from model-visible content.
   */
  readonly submission?: 'routing'
...
/** Identity and cancellation state shared by one routed composer submission. */
export interface RoutingSubmitAttempt {
  readonly seq: number
  readonly signal: AbortSignal
}

/** Plugin-owned routed submission after generic reference serialization. */
export interface RoutingSubmitRequest {
  readonly source: string
  readonly ref: string
  readonly content: string
  readonly images: readonly SubmitImageAttachment[]
  readonly mode: 'queue' | 'steer'
  readonly attempt: RoutingSubmitAttempt
}
...
export interface InputTriggerSource {
  ...
  /**
   * Submit a task claimed by this source's leading routing reference.
   * Failure or abort leaves the source composer intact.
   */
  routeSubmit?(session: ClientSessionContext, request: RoutingSubmitRequest): Promise<SubmitOutcome>
}
