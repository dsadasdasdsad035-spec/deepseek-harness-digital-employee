...
import type {
  ArbitrateKey, ArbitrateOutcome, ClientSessionContext, PickOutcome, InputTriggerSource,
  RoutingSubmitRequest, SubmitEnvelope, SubmitOutcome, TriggerChar, TriggerGuard,
} from '../types.ts'
...
export class InputTriggerController {
  ...
  /**
   * Route one submission through the source that owns its leading reference.
   * @param request - route identity, content, attachments, and attempt.
   * @returns the source-owned submission outcome.
   */
  routeSubmit(request: RoutingSubmitRequest): Promise<SubmitOutcome> {
    const owner = this.deps.roster.all().find(source => source.name === request.source)
    if (owner?.routeSubmit === undefined) {
      return Promise.reject(new Error(`slash: no route submitter for reference source "${request.source}"`))
    }
    return owner.routeSubmit(this.project(), request)
  }
