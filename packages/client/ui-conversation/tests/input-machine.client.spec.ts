...
function routeOf(name: string, source = 'digital-employee'): ReferenceInsert {
  return { ...refOf(name, source), submission: 'routing' }
}
...
describe('input-machine: insert-ref and the occurrence table', () => {
  ...
  it('accepts one routing reference only at the leading semantic position', () => {
    const m = new InputMachine()
    m.dispatch({ type: 'draft-changed', draft: '  @ops' })
    m.dispatch({ type: 'insert-ref', reference: routeOf('Operations'), span: spanOf(m, 2, 6) })
    expect(m.state.occurrences).toEqual([
      expect.objectContaining({ source: 'digital-employee', ref: 'Operations', submission: 'routing', offset: 2 }),
    ])

    const owner = m.state.occurrences[0]
    const draft = `${m.state.draft}@review`
    m.dispatch({
      type: 'draft-changed',
      draft,
      editRange: { start: m.state.draft.length, end: m.state.draft.length, insertedLength: 7 },
    })
    const rev = m.state.draftRev
    m.dispatch({
      type: 'insert-ref',
      reference: routeOf('Reviewer'),
      span: spanOf(m, draft.length - 7, draft.length),
    })
    expect(m.state.draftRev).toBe(rev)
    expect(m.state.occurrences).toEqual([owner])

    const inline = new InputMachine()
    inline.dispatch({ type: 'draft-changed', draft: 'ask @ops' })
    inline.dispatch({ type: 'insert-ref', reference: routeOf('Operations'), span: spanOf(inline, 4, 8) })
    expect(inline.state.occurrences).toEqual([])
    expect(inline.state.draft).toBe('ask @ops')
  })

  it('invalidates routing ownership when editing moves content before it, with undo and redo restoring each state', () => {
    const m = new InputMachine()
    m.dispatch({ type: 'draft-changed', draft: '@ops' })
    m.dispatch({ type: 'insert-ref', reference: routeOf('Operations'), span: spanOf(m, 0, 4) })
    const routedDraft = m.state.draft
    expect(m.state.occurrences).toHaveLength(1)

    m.dispatch({
      type: 'draft-changed',
      draft: `ask ${routedDraft}`,
      editRange: { start: 0, end: 0, insertedLength: 4 },
    })
    expect(m.state.occurrences).toEqual([])
    expect(m.state.draft).toBe(`ask ${routedDraft}`)
    ...
    expect(m.state).toMatchObject({ draft: `ask ${routedDraft}`, occurrences: [] })
  })

  it('invalidates routing ownership when a public insert-ref event places an ordinary reference before it', () => {
    const m = new InputMachine()
    m.dispatch({ type: 'draft-changed', draft: '@ops' })
    m.dispatch({ type: 'insert-ref', reference: routeOf('Operations'), span: spanOf(m, 0, 4) })
    m.dispatch({ type: 'insert-ref', reference: refOf('Research', 'reference'), span: spanOf(m, 0, 0) })
    expect(m.state).toMatchObject({
      draft: '@Research @Operations ',
      occurrences: [
        expect.objectContaining({ ref: 'Research', source: 'reference' }),
      ],
    })
  })
})

describe('input-machine: routed submit', () => {
  it('claims the routing occurrence without serializing its display token into task content', () => {
    const m = new InputMachine()
    m.dispatch({ type: 'draft-changed', draft: '@ops' })
    m.dispatch({ type: 'insert-ref', reference: routeOf('Operations'), span: spanOf(m, 0, 4) })
    m.dispatch({ type: 'draft-changed', draft: `${m.state.draft}review release` })

    const effect = effectAt(m.dispatch({ type: 'enter', mode: 'queue' }), 0, 'route-submit')
    expect(effect).toMatchObject({
      route: { source: 'digital-employee', ref: 'Operations' },
      content: 'review release',
      mode: 'queue',
    })
    expect(effect.attempt.draftSnapshot).toBe('@Operations review release')
    expect(m.dispatch({ type: 'enter', mode: 'queue' })).toEqual([])
  })
})
...
describe('input-machine: paste plane', () => {
  it('keeps an inline synchronous routing component as plain pasted text', () => {
    const m = new InputMachine()
    m.dispatch({ type: 'draft-changed', draft: 'ask ' })
    m.dispatch({
      type: 'paste-begin',
      text: '@ops',
      selection: { start: 4, end: 4 },
      components: [{ start: 0, end: 4, reference: routeOf('Operations') }],
    })
    expect(m.state).toMatchObject({ draft: 'ask @ops', occurrences: [] })
  })

  it('rejects an asynchronous routing upgrade outside the leading semantic position', () => {
    const m = new InputMachine()
    m.dispatch({ type: 'paste-begin', text: 'ask @ops', selection: { start: 0, end: 0 } })
    ...
    m.dispatch({
      ...
      attemptId: 1,
      span: spanOf(m, 4, 8),
      reference: routeOf('Operations'),
    })
    expect(m.state).toMatchObject({ draft: 'ask @ops', draftRev: rev, occurrences: [] })
  })

  it('invalidates routing ownership when an asynchronous ordinary upgrade lands before it', () => {
    const m = new InputMachine()
    m.dispatch({ type: 'draft-changed', draft: '  @ops' })
    m.dispatch({ type: 'insert-ref', reference: routeOf('Operations'), span: spanOf(m, 2, 6) })
    m.dispatch({ type: 'paste-begin', text: '', selection: { start: m.state.draft.length, end: m.state.draft.length } })
    m.dispatch({
      type: 'paste-upgrade',
      attemptId: 1,
      span: spanOf(m, 0, 1),
      reference: refOf('Research', 'reference'),
    })
    expect(m.state.occurrences).toEqual([
      expect.objectContaining({ ref: 'Research', source: 'reference' }),
    ])
  ...
  it('invalidates an existing routing reference when plain paste inserts task content before it', () => {
    const m = new InputMachine()
    m.dispatch({ type: 'draft-changed', draft: '@ops' })
    m.dispatch({ type: 'insert-ref', reference: routeOf('Operations'), span: spanOf(m, 0, 4) })
    const routedDraft = m.state.draft
    m.dispatch({ type: 'paste-begin', text: 'ask ', selection: { start: 0, end: 0 } })
    expect(m.state).toMatchObject({ draft: `ask ${routedDraft}`, occurrences: [] })
  })
