...
describe('running and lock semantics', () => {
  ...
  it('a programmatic first reference leaves immediate typing after the token', () => {
    ...
    fireEvent.change(textarea, { target: { value: '@测试 task' } })
  ...
  it('a first inline reference does not move the caret to the end of an existing draft', () => {
    const { textarea, shell } = bench({ draft: 'alpha @f omega' })
    ...
    act(() => {
      shell.insertReference({
        ...
        clipboardText: '@f',
        submission: 'routing',
      }, { start: 6, end: 8, draftRev: shell.snapshot.draftRev })
    })
    expect(textarea.value).toBe('alpha @f omega')
    expect(textarea.selectionStart).not.toBe(textarea.value.length)
    expect(textarea.selectionEnd).not.toBe(textarea.value.length)
  })
