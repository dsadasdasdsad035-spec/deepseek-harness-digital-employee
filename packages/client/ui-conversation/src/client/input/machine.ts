...
export class InputMachine {
  ...
  private mint(reference: ReferenceInsert, offset: number, length: number): Occurrence {
    ...
    return {
      ...
      ...reference.submission === undefined ? {} : { submission: reference.submission },
    }
  }
  ...
  private onInsertRef(reference: ReferenceInsert, span: TokenSpan): InputEffect[] {
    ...
    if (!this.canInsertReference(reference, this.draft.slice(0, span.start), this.occurrences)) return []
    this.replaceSpanWithChip(reference, span)
    this.paste = undefined
    return []
  }

  /** Enforce leading-only and single-owner semantics at every reference insertion path. */
  private canInsertReference(
    reference: ReferenceInsert,
    prefix: string,
    occurrences: readonly Occurrence[],
  ): boolean {
    return reference.submission !== 'routing'
      || (prefix.trim() === '' && !occurrences.some(occurrence => occurrence.submission === 'routing'))
  }

  /** Drop route ownership when ordinary content moves before its occurrence. */
  private invalidateRouting(): void {
    this.occurrences = this.occurrences.filter((occurrence) => {
      return occurrence.submission !== 'routing'
        || this.draft.slice(0, occurrence.offset).trim() === ''
    })
  }
  ...
  private onPasteBegin(
    ...
    const retained = this.occurrences.filter((occurrence) => {
      return occurrence.offset + occurrence.length <= start || occurrence.offset >= end
    })
    let inserted = ''
    let cursor = 0
    for (const c of sorted) {
      const before = inserted + text.slice(cursor, c.start)
      if (!this.canInsertReference(
        c.reference,
        this.draft.slice(0, start) + before,
        [...retained, ...minted],
      )) continue
      inserted = before
  ...
  private onEnter(mode: InputSubmitMode): InputEffect[] {
    ...
    if (trimmed === '') return []
    this.paste = undefined
    const route = this.occurrences.find(occurrence => occurrence.submission === 'routing')
    if (route !== undefined) {
      const attempt = this.beginAttempt(mode)
      this.phase = 'submitting'
      const content = (
        this.draft.slice(0, route.offset)
        + this.draft.slice(route.offset + route.length)
      ).trim()
      return [{
        type: 'route-submit',
        attempt,
        route: { source: route.source, ref: route.ref },
        content,
        mode,
      }]
    }
