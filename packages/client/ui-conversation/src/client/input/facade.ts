...
export class SessionInputShell implements SessionInput {
  ...
  private execute(fx: InputEffect): void {
    switch (fx.type) {
      ...
      case 'route-submit': {
        this.routeSerialized(fx.attempt, fx.route, fx.content, fx.mode)
        return
      }
      default:
  ...
  /** Prepare generic references and attachments before invoking the route owner. */
  private routeSerialized(
    attempt: SubmitAttempt,
    route: { readonly source: string; readonly ref: string },
    content: string,
    mode: InputSubmitMode,
  ): void {
    const imageIds = [...this.imageIds]
    void (async () => {
      try {
        ...
        if (inputTriggers === undefined) {
          throw new Error(`no route submitter for reference source "${route.source}"`)
        }
        const [serializedContent, images] = await Promise.all([
          this.serializeRoutedContent(attempt, content, inputTriggers),
          this.deps.commandImages.serialize(imageIds),
        ])
        if (this.dead(attempt)) return
        this.settleRoutedSubmit(attempt, inputTriggers.routeSubmit({
          source: route.source,
          ref: route.ref,
  ...
  /** Replace model references and omit the routing occurrence from prompt text. */
  private async serializeRoutedContent(
    attempt: SubmitAttempt,
    content: string,
    inputTriggers: InputTriggerController,
  ): Promise<string> {
    const occurrences = this.core.state.occurrences
    if (occurrences.length === 1) return content
    let out = ''
    let cursor = 0
    for (const occurrence of occurrences) {
      out += attempt.draftSnapshot.slice(cursor, occurrence.offset)
      if (occurrence.submission !== 'routing') {
        out += await inputTriggers.serializeReference(occurrence.source, occurrence.ref, attempt.signal)
      }
      cursor = occurrence.offset + occurrence.length
    }
    return (out + attempt.draftSnapshot.slice(cursor)).trim()
  }

  /** Commit routed attachments only after the route owner accepts the attempt. */
  private settleRoutedSubmit(
    attempt: SubmitAttempt,
