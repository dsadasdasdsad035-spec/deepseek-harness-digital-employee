...
export function InputBar({
  ...
  const draftWasEmptyRef = useRef(draft === '')
  ...
  // A persisted draft or programmatic reference arrives AFTER the unlock
  // effect. Put the selection after that first content so immediate typing
  // appends to it, then reveal the caret. This effect does not focus:
  // send-clear, failed-send restore, and first-character transitions must not
  // steal focus from another control the user moved to.
  useEffect(() => {
    const el = inputRef.current
    const firstContent = draftWasEmptyRef.current && draft !== ''
    draftWasEmptyRef.current = draft === ''
    if (locked || draft === '' || el === null) return
    if (firstContent) el.setSelectionRange(draft.length, draft.length)
    revealSelectionFocus(el)
  }, [draft !== ''])
