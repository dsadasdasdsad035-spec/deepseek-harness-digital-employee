...
/** Per-session standard props resolved per session id (identity-stable per session scope). */
export interface SessionMaybeProvideInfo {
   ...
   * Unlike `hooks`, the key space is open — values arrive from host-computed
   * push frames — so the render side binds per resolved key instead of per
   * static roster member. Faces are always defined per key; the whole member
   * is absent with the session.
   */
