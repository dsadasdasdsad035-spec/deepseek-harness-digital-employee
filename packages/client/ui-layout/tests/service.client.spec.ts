...
describe('LayoutController', () => {
  it('forwards the three panel actions to the attached set', () => {
    ...
    expect(panels.openApplication).toHaveBeenCalledTimes(1)
    expect(panels.closeApplication).toHaveBeenCalledTimes(1)
  ...
  it('fails loud before the root entry wired its actions', () => {
    ...
    expect(() => { service.openApplication() }).toThrow(/panel actions not wired/)
    expect(() => { service.closeApplication() }).toThrow(/panel actions not wired/)
  })
