...
describe('ui-layout client apply', () => {
  ...
  it('provides ctx.layout and registers AppFrame into root with the three child declarations', async () => {
    ...
    expect(slots.spec('shell.application')).toEqual({ kind: 'single', scope: 'root' })
  })
  ...
  it('injects no business face and attaches the layout actions', async () => {
    ...
    const actions = {
      ...
      openDetails: vi.fn(), closeDetails: vi.fn(), openApplication: vi.fn(), closeApplication: vi.fn(),
    }
    const injected = (slots.entries('root')[0]!.inject as
      (actions: never) => { layout: LayoutController })(actions as never)
    // oxlint-disable-next-line typescript/no-unsafe-assignment -- Vitest asymmetric matchers are intentionally typed as any
    expect(injected).toEqual({ layout: expect.any(LayoutController) })
    injected.layout.attachPanels(actions)
