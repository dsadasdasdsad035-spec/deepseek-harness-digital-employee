...
function mountFrame() {
  ...
  const renderSlot = ((key: string, owner: object) => {
    ...
    if (key === 'shell.application') return <div data-testid="application-content" />
...
describe('AppFrame', () => {
  ...
  it('shows the application workspace without conversation or details chrome', () => {
    const { frame, instance, queryByTestId, getByTestId } = mountFrame()
    act(() => { instance.actions.openApplication() })

    expect(getByTestId('application-content')).toBeTruthy()
    expect(queryByTestId('center-content')).toBeNull()
    expect(queryByTestId('details-content')).toBeNull()
    expect(tracks(frame)).toEqual([280, 0])

    act(() => { instance.actions.closeApplication() })
    expect(getByTestId('center-content')).toBeTruthy()
    expect(getByTestId('details-content')).toBeTruthy()
    expect(queryByTestId('application-content')).toBeNull()
  })
