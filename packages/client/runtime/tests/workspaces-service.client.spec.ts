...
describe('WorkspaceRuntime', () => {
  ...
  it('can force a distinct blank session instead of reusing an existing workspace draft', async () => {
    const ctx = new Context()
    const api = new FakeApiClient()
    const sessions = new SessionRuntime(ctx, api, fakeRemote())
    const workspaces = new WorkspaceRuntime(ctx, api, sessions)
    api.onWorkspaceList = () => Promise.resolve(ok({
      items: [workspace('one', [sid('existing')])] as never[],
    }))
    api.onList = () => Promise.resolve(ok({ items: [{
    ...
    await expect(workspaces.connectWorkspace(wid('one'))).resolves.toBe(sid('existing'))
    await expect(workspaces.connectWorkspace(wid('one'), { reuseBlank: false })).resolves.toBe(sid('fresh'))
    expect(api.callsOf('session.create')).toEqual([{ workspaceId: 'one' }])
  })
