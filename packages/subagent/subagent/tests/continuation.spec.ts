...
describe('SubagentRuntime.startContinuable', () => {
  ...
  it('persists detached composition and replays it before fresh and resumed child publication', async () => {
    const { ctx, parent } = await setup([textResponse('first'), textResponse('resumed')])
    const seen: Array<{ childId: string; data: unknown; published: boolean }> = []
    ctx.on('subagent/compose', (childCtx, data) => {
      const child = childCtx.agent as Agent
      seen.push({
        childId: child.id,
        data,
        published: ctx.agents.get(child.id) !== undefined,
      })
    })
    const composition = {
      digitalEmployee: {
    ...
    const started = await ctx.subagents.startContinuable({
      ...startSpec(parent),
      request: {
        prompt: message('composed work'),
        parent,
        composition,
      },
    })
    composition.digitalEmployee.mcpServerIds.push('unauthorized')
    await waitNoActivation(ctx, started.childId)
    ...
    await followup(ctx, parent, started.childId, message('resume composed work'))
    await waitNoActivation(ctx, started.childId)
    expect(seen).toEqual([
      seen[0],
      {
