...
export interface ChildComposition {
  ...
  /** Non-secret plugin composition data dispatched before child publication. */
...
export async function applyChildComposition(
  ...
  if (composition.data !== undefined) {
    await childCtx.serial('subagent/compose', childCtx, composition.data)
  }
}
