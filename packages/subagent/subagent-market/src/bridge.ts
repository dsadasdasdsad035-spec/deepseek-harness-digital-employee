...
import type { InstalledSubagentPackage } from './index.ts'

/** One resolved binding: an installed package plus one of its personas. */
export interface EmployeeSubagentBinding {
  readonly pkg: InstalledSubagentPackage
  readonly persona: InstalledSubagentPackage['descriptor']['subagents'][number]
}

/** Options for mounting employee subagent personas. */
export interface MountEmployeeSubagentsOptions {
  /** Unused today; reserved for per-employee model routing. */
...
export function mountEmployeeSubagents(
  agentCtx: Context,
  bindings: readonly EmployeeSubagentBinding[],
  _options: MountEmployeeSubagentsOptions = {},
): () => void {
