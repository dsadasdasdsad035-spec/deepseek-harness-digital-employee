...
import { ArchiveValidationError, decodeArchiveBase64, descriptorSignaturePayload, inspectZipArchive, listManagedPackages, parseSubagentPackageDescriptor, preparePackageArchive, publishManagedPackage, readManagedPackage, resolveTrustedPublisher, uninstallManagedPackage, verifyPackageFileHashes, verifyPublisherSignature, } from '@deepseek-ai/dsh-marketplace-core';
 ...
 */
export function asSubagentMarketResult(error) {
    if (error instanceof SubagentMarketDomainError)
...
export class SubagentMarketService {
    ...
    async descriptor(packageId) {
        try {
            ...
            if (ownership.status !== 'managed')
                throw new Error('subagent package is not marketplace-managed');
            ...
            const entries = await Promise.all(Object.keys(descriptor.files).map(async (name) => {
                ...
                if (!info.isFile() || info.isSymbolicLink())
                    throw new Error(`subagent package file "${name}" is not regular`);
    ...
    async install(request) {
        return await result(async () => {
            const archive = preparePackageArchive(await inspectZipArchive(decodeArchiveBase64(request.archiveBase64)), 'subagent-package.json');
            const descriptorEntry = archive.entries.find(entry => entry.name === 'subagent-package.json');
            if (descriptorEntry === undefined)
                throw new Error('archive must contain subagent-package.json');
        ...
        return await result(async () => {
            return await this.mutations.runExclusive(packageId, async () => {
                ...
                if (ownership.status === 'missing')
                    throw new SubagentMarketDomainError({ code: 'not-found', packageId });
                if (ownership.status === 'unmanaged')
                    throw new SubagentMarketDomainError({ code: 'unmanaged-conflict', packageId });
                if (ownership.status === 'incompatible')
                    throw new SubagentMarketDomainError({ code: 'manifest-incompatible', packageId });
