...
export interface InProcessRunOptions {
  ...
  /**
   * Extra composition for the child's creation window, after the driver's own
   * persona and tool-filter application. Providers owning domain-specific
   * child scoping (per-expert MCP servers, scoped sections) register it here.
   */
  readonly composeChild?: (childCtx: Context, child: Agent) => void | Promise<void>
}
...
export async function startInProcessRun(
  ...
  const setup = async (childCtx: Context, child: Agent): Promise<void> => {
    ...
    try {
      await options.composeChild?.(childCtx, child)
    } catch (error: unknown) {
