...
describe('startInProcessRun', () => {
  ...
  it('awaits one-shot composition before publishing the child', async () => {
    const { ctx, parent } = await setup([textResponse('driver answer')])
    ...
    const starting = startInProcessRun({
      ...request(parent),
      composition: { digitalEmployee: { employeeId: 'employee-1' } },
    }, {})
    ...
    expect(seen).toEqual([{ digitalEmployee: { employeeId: 'employee-1' } }])
    expect(ctx.agents.get(run.id)).toBeDefined()
    await run.result
    await run.dispose()
  })
