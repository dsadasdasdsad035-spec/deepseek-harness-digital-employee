...
export async function mountAcpMcpServers(
  ...
  for (const config of configs) {
    // The plugin's apply accepts the pre-defaulted input form; the resolved
    // configs here are complete inputs, so structural identity holds.
    await agentCtx.plugin(McpClient, config as never)
  }
}
