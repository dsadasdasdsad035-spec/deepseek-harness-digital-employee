...
import type { McpServerConfig as McpClientConfig, StdioConfig, StreamableHttpConfig } from '@deepseek-ai/dsh-mcp-client'
...
describe('ACP MCP declaration mapping', () => {
  it('normalizes human server names and preserves standard stdio/HTTP fields', async () => {
    ...
    expect(configs[0] as StdioConfig).toMatchObject({
      transport: 'stdio',
      command: process.execPath,
      args: ['server.js'],
      env: { TOKEN: 'secret' },
    ...
    expect((configs[0] as StdioConfig)?.serverName).toMatch(/^Fancy_server_[0-9a-f]{8}$/)
    expect(configs[1] as StreamableHttpConfig).toMatchObject({
      transport: 'streamable-http',
      url: 'https://example.test/mcp',
      headers: { Authorization: 'Bearer token' },
      failOnStartupError: true,
    })
    expect((configs[1] as StreamableHttpConfig)?.serverName).toMatch(/^server_[0-9a-f]{8}$/)
  })
  ...
  it('preserves legal names that collide with Object prototype setters', async () => {
    ...
    expect(configs[0]?.transport === 'stdio' && Object.hasOwn((configs[0] as StdioConfig).env, '__proto__')).toBe(true)
    expect(configs[0]?.transport === 'stdio' && (configs[0] as StdioConfig).env['__proto__']).toBe('environment-value')
    expect(configs[1]?.transport === 'streamable-http' && Object.hasOwn((configs[1] as StreamableHttpConfig).headers, '__proto__')).toBe(true)
    expect(configs[1]?.transport === 'streamable-http' && (configs[1] as StreamableHttpConfig).headers['__proto__']).toBe('header-value')
  })
