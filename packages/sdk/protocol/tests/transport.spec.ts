...
const digitalEmployeeIdentity = {
  ...
  templateId: createDigitalEmployeeTemplateId('template-1'),
...
} satisfies DigitalEmployeeIdentityEvent
const digitalEmployeeNotification: HarnessSdkNotificationMap['session.event'] = {
  sessionId: 'session-1',
  event: {
...
describe('JsonRpcLineTransport', () => {
  it('projects digital employee events through the SDK notification type', () => {
    expect(digitalEmployeeNotification.event.type).toBe('digital-employee/identity')
  })
