 ...
 * - `FAKE_DIGITAL_EMPLOYEE_EVENTS`: emit one required digital employee event
 *   and one unknown event explicitly marked ignorable before the normal turn.
...
reader.on('line', (line) => {
  ...
  switch (frame.method) {
    ...
    case 'session/prompt': {
      ...
      if (env.FAKE_DIGITAL_EMPLOYEE_EVENTS !== undefined) {
        event(sessionId, 'digital-employee/identity', {
          employeeId: 'employee-1',
          templateId: 'release-engineer',
        ...
        notify('session.event', {
          sessionId,
          event: {
            type: 'future/employee-extension',
            seq: seq++,
            time: 0,
            data: { revision: 2 },
            ignorable: true,
