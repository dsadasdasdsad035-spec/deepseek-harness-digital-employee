...
const requiredArtifacts = [
  ...
  'packages/skill/skill-market/lib/typert.remote-client.js',
...
describe.skipIf(!requiredArtifacts)('Goal Remote built LIB chain', () => {
  it('runs root and Agent-scoped calls through generated bundles and real HTTP', async () => {
    ...
    const script = `
      ...
      const result = {
        digitalEmployeeHasStartChat: typeof client.remote.digitalEmployees.startChat === 'function',
        digitalEmployeeHasRunTask: 'runTask' in client.remote.digitalEmployees,
        invalidRejected,
    ...
    const output = JSON.parse(result.stdout.trim().split('\n').at(-1) ?? '{}') as {
      digitalEmployeeHasStartChat: boolean
      digitalEmployeeHasRunTask: boolean
    ...
    expect(output).toMatchObject({
      digitalEmployeeHasStartChat: true,
      digitalEmployeeHasRunTask: false,
      invalidRejected: true,
