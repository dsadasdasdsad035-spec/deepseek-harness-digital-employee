...
class RemoteNamespaceService extends Service {
  ...
  installDirect(descriptor: InvocationDescriptor, token: MountToken): void {
    this.installMethod(descriptor.method, 'direct', { descriptor, token })
  }

  installScoped(descriptor: InvocationDescriptor, projection: ScopedProjection, token: MountToken): void {
    this.installMethod(descriptor.method, 'scoped', { descriptor, projection, token })
  }

  private installMethod(method: string, kind: 'direct', value: DirectMethod): void
  private installMethod(method: string, kind: 'scoped', value: ScopedMethod): void
  private installMethod(method: string, kind: 'direct' | 'scoped', value: DirectMethod | ScopedMethod): void {
