...
function installDescriptor(): InvocationDescriptor {
  return {
    id: '@fixture/market#market/install',
    service: 'market',
    namespace: 'market',
    method: 'install',
    invocation: { kind: 'direct' },
...
describe('Client Typert API', () => {
  it('mounts an install business operation through the shared API carrier and withdraws it', async () => {
    const call = vi.fn<ConnectionHandle['rpc']['call']>()
      .mockResolvedValue({ ok: true, value: { ref: 'skill-1' } })
    const ctx = await bench(call)
    const dispose = await ctx.remote.$mount({
      package: '@fixture/market',
      descriptors: [installDescriptor()],
    })
    ...
    await expect(market.install({ objective: 'ship' }))
      .resolves.toEqual({ ok: true, value: { ref: 'skill-1' } })
    expect(call).toHaveBeenCalledWith(
      '/api',
      'market/install',
      { args: { request: { objective: 'ship' } } },
      expect.any(AbortSignal),
    )

    await dispose()
    expect((ctx.remote as unknown as Record<string, unknown>).market).toBeUndefined()
    await expect(retained({ objective: 'ship' })).resolves.toMatchObject({
      ok: false,
      error: { message: 'client api: Remote method market/install is no longer mounted' },
    })
  })
