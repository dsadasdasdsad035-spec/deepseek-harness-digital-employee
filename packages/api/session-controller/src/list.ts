...
export class ApiSessionList {
  ...
  async list(signal?: AbortSignal): Promise<SessionSummary[]> {
    ...
    for (const record of records) {
      // Preview sessions back ephemeral employee drafts and stay invisible to
      // the ordinary session list and search.
      if (record.header.preview === true) continue
      const live = this.ctx.sessions.get(record.header.id)
      if (live !== undefined) {
        if (live.header.preview === true) continue
  ...
  async search(query: string, signal: AbortSignal): Promise<SessionSearchValue> {
    ...
    try {
      ...
      const visibleIds = new Set(visible
        .filter(record => record.header.cwd !== undefined && record.header.preview !== true)
