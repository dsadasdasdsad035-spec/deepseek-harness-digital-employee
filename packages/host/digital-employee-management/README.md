...
Remote method names are unique within `digitalEmployees` and describe management actions directly: `listTemplates`, `listEmployeeTasks`, `resumeEmployeeTask`, `discardEmployeeTask`, `describeNotificationChannels`, `testNotificationChannel`, `list`, `get`, `create`, `activate`, `deactivate`, `delete`, `startChat`, `listMemory`, `deleteMemory`, `listExperts`, `taskTree`, `continueExpert`, `interruptExpert`, `listAudit`, `previewUpgrade`, `applyUpgrade`, `exportEmployee`, and `importEmployee`.

The task-console remotes read and write the autonomous-task attempt ledger under the same cross-process file lock the headless task driver uses: `listEmployeeTasks` folds the ledger into console rows (display names fall back to the task key for legacy records), `resumeEmployeeTask` clears the suspension flag of a suspended key while keeping its failure count, and `discardEmployeeTask` removes the record entirely. The channel remotes surface the notification seam for settings: `describeNotificationChannels` reports each registered channel's credential configuration facts (never values), and `testNotificationChannel` sends one `[test]`-labeled message through the production delivery contract. Namespace and method names are separate RPC path segments and may use the same domain vocabulary. Client namespace implementation members use bookkeeping-specific names so generated business methods are not accidentally reserved.
