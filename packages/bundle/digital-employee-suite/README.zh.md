...
用户与受管理的数字员工开始聊天时（`@数字员工` 输入源或管理页），组合后的 Host 会解析该员工的已发布模板、指令、声明的技能、工具、MCP 服务、专家和记忆投影，并追加记录该组合的持久化 `digital-employee/identity` 事件。suite 提供管理与聊天入口；Web bundle 提供共享的市场和 API remote 组合。
