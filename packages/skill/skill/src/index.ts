...
/** Scope-local filter applied to the complete visible skill catalog. */
export interface SkillRestriction {
  /** Skill names retained in the viewing scope. Empty denies every skill. */
...
class SkillLayer implements ScopeLayer {
  ...
  /** Intersecting allowlists registered through this exact scope. */
  readonly restrictions = new Set<ReadonlySet<string>>()
  ...
  isEmpty(): boolean {
    return this.providers.isEmpty() && this.runtime.size === 0 && this.restrictions.size === 0
  }
}
...
export class SkillRegistry extends Service {
  ...
  restrict(filter: SkillRestriction): () => void {
    const scope = scopeOf(this.ctx)
    if (scope === undefined) {
      throw new Error('skills.restrict() requires a scoped context (agent.ctx)')
    }
    for (const name of filter.allow) {
      if (!isSkillName(name)) throw new Error(`skills.restrict() names invalid skill ${JSON.stringify(name)}`)
    }
    const allow = new Set(filter.allow)
    return this.layers.effect(
      this.ctx,
      (layer) => {
        layer.restrictions.add(allow)
        return () => { layer.restrictions.delete(allow) }
      },
  ...
  private async collectFresh(options: SkillViewOptions): Promise<CollectResult> {
    ...
    for (const layer of this.layers.chainLayers(options.scope)) {
      for (const allow of layer.restrictions) {
