...
describe('SkillRegistry scoped layers', () => {
  it('restricts the complete skill view to a scoped allowlist and removes the restriction on disposal', async () => {
    const ctx = new Context()
    await ctx.plugin(SkillRegistry)
    ctx.skills.register({ name: 'allowed', description: 'Allowed', source: 'test', content: 'Allowed.' })
    ctx.skills.register({ name: 'ambient', description: 'Ambient', source: 'test', content: 'Ambient.' })
    const key = { employee: 'alpha' }
    const employee = createScope(ctx, key)

    scopedSkills(employee.ctx).restrict({ allow: ['allowed'] })

    expect((await ctx.skills.list({ scope: key })).map(skill => skill.name)).toEqual(['allowed'])
    expect((await ctx.skills.list()).map(skill => skill.name)).toEqual(['allowed', 'ambient'])
