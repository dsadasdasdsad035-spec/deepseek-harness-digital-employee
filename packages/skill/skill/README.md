...
- `agent.ctx.skills.restrict({ allow }): () => void` Restricts the complete catalog visible through that Agent scope after global, preset, and nearer scoped entries merge. Multiple allowlists intersect, an empty list denies every skill, and disposing the owning fiber removes the restriction.
